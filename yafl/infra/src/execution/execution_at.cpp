...
{
    INFRA_LOG_DEBUG( "old t1: ", _t1 );
    INFRA_LOG_DEBUG( "old t2: ", _t2 );
    INFRA_LOG_DEBUG( "new t1: ", t1 );
    INFRA_LOG_DEBUG( "new t2: ", t2 );
...
bool ExecutionAt::mainExecution()
{
    ...
    { 
        // sleep t1
        {
            INFRA_LOG_DEBUG( "now going to sleep till (t1) ", _t1 );
        ...
        { 
            ...
            {            
                while ( ! isStopped() )
                {
                    ...
                    {
                        {
                            INFRA_LOG_DEBUG( "now going to sleep till (t2) ", _t2 );
                            std::unique_lock< std::mutex > lk( _mutex ); 
    ...
    INFRA_LOG_EXIT( std::boolalpha, retVal ); 
    return retVal;
}
